<?php
require_once "ad_lister_credentials.php";
abstract class Model
{
    ...
    {
        if (!self::$dbc) { 
        // connct to database and checking if null...if we don't have a db connection make a new one
    ...
    public function save()
    {
        ...
        if(!empty($this->attributes)){
            if(isset($this->attributes['id'])){
                $this->update(); // update($this->attributes['id']);
            }else{
                $this->insert();
            }
        }
